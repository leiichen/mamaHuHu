...
// 渲染短视频 AI 生大纲 Tab 内容区
export function VideoAiTabContent() {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            navigate(getProjectPagePath(draft.projectId, "video"), {
    ...
    return (
        <div className="xyq-video-ai-panel flex h-full min-h-0 flex-col gap-3">
            <textarea
                value={storyText}
                onChange={(event) => setStoryText(event.target.value)}
                placeholder="在此输入你构想的短视频创意。可以尝试输入：主题设定、画面风格、节奏与情绪、想要呈现的画面等等"
                className="xyq-video-textarea h-[144px] w-full shrink-0 resize-none rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm leading-6 text-slate-800 outline-none placeholder:text-slate-400 focus:border-violet-300"
                disabled={creating}
            />
